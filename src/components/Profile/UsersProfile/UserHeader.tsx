...
export default function UserHeader(props: PropsType) {
    ...
    return (
        <div className='container mx-auto px-2 md:px-5'>
            <div className="profile-header shadow rounded relative">
                ...
                }
                {
                    selectedUser?.avatar && <div className={`border-2 border-white shadow-md w-40 h-40 rounded-full absolute left-7 -bottom-8 z-[100] overflow-hidden ${!selectedUser?.avatar && "bg-slate-400 flex justify-center items-center"}`}>
                        {
                            selectedUser?.avatar ? <img className='w-full h-full object-cover' src={selectedUser?.avatar} alt="family" /> : <h1 className='text-sm font-semibold'>Not Added</h1>
                        }
                    </div>
                }
                {
            ...
            <div className="flex flex-col lg:flex-row w-full bg-white shadow justify-center items-center">
                <div className="lg:py-6 pt-6 pb-2 lg:ps-[200px] lg:justify-self-start lg:flex-1">
                    {
                        selectedUser && <div className='text-center lg:text-start'>
